      ...
      <section className={`${dc.shell.section} ${dc.shell.sectionY}`}>
        {posts.length ? (
          ...
          <div className={`${dc.surface.soft} p-8 text-center`}>
            <h2 className="text-3xl font-black tracking-[-0.05em]">No image notes found</h2>
            <p className={`mt-3 text-sm leading-7 ${pal.softMutedText}`}>Try another category or return to all image and profile posts.</p>
          </div>
        )}
    ...
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-10 sm:pt-14 lg:pt-16`}>
        <div className={`grid gap-6 rounded-[2.5rem] border ${pal.border} bg-white p-6 shadow-[0_24px_80px_rgba(24,20,17,0.08)] lg:grid-cols-[minmax(0,1fr)_320px] lg:p-10`}>
          <div className="min-w-0">
            <Link href="/image" className={`inline-flex items-center gap-2 rounded-full border ${pal.border} px-4 py-2 text-sm font-black ${pal.panelText}`}><ChevronLeft className="h-4 w-4" /> Images</Link>
          ...
          <aside className={`min-w-0 rounded-[2rem] ${pal.darkBg} p-6 text-white`}>
            <p className={`${dc.type.eyebrow} ${pal.accentSoftText}`}>Profile note</p>
      ...
      <section className="mx-auto w-full max-w-5xl px-4 pb-16 pt-6 sm:px-6 lg:px-8 lg:pb-24">
        <div className={`rounded-[2.25rem] border ${pal.border} bg-white p-6 shadow-[0_24px_80px_rgba(24,20,17,0.08)] sm:p-8 lg:p-10`}>
          <p className={`text-sm leading-8 ${pal.softMutedText}`}>{post?.summary || `Image and profile detail content for ${slug} will render through the editable detail page.`}</p>
        </div>
      </section>
    </main>
  )
