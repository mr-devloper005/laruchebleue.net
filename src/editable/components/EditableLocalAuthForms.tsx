...
const inputClass = 'h-13 rounded-lg border border-white/12 bg-white/[0.055] px-4 text-base font-bold text-white outline-none transition placeholder:text-white/35 focus:border-[var(--slot4-accent)] focus:bg-white/[0.08]'
const buttonClass = 'inline-flex h-13 items-center justify-center rounded-lg bg-[var(--slot4-accent-fill)] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5 hover:shadow-[0_18px_45px_rgba(91,0,255,0.38)] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-lg px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-400/15 text-emerald-200' : 'bg-red-400/15 text-red-200'}`}>{message}</p> : null}
